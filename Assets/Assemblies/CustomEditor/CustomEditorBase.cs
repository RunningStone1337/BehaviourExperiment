...
{
    private static string abstractTypeFullName;
    ...
    protected int DrawAddSubtractButtonsForValue(string label, int value, int minValue, int maxValue)
    {
        BeginHorizontal("box");
        if (GUILayout.Button("-"))
            --value;
    ...
    protected void CheckDirty<T>() where T : Object
    {
        if (GUI.changed)
            EditorUtility.SetDirty((T)target);
    }
    ...
    /// <param name="storedIndexes">Лист индексов для каждого выпадающего списка</param>
    ...
    protected (List<string> fullNames, List<int> indexes, List<string> shortNames)
        DrawTypesSelectors(List<Type> types, List<string> listNames, List<int> storedIndexes, List<string> listShortNames,
        Action afterAddingDelegate = null, Action<int> afterDrawingSelectorDelegate = null, Action<int> afterRemovingSelectorDelegate = null)
    {
        ...
        if (storedIndexes == null)
            storedIndexes = new List<int>();
        if (listShortNames == null)
            listShortNames = new List<string>();
        if (listNames.Count > 0)
        {
            if (listNames.Count == storedIndexes.Count && storedIndexes.Count == listShortNames.Count)
            {
                for (int index = 0; index < listNames.Count; index++)
                {
                    GUILayout.BeginHorizontal("box");
                    (listNames[index], storedIndexes[index], listShortNames[index]) = DrawTypeSelector(types, listNames[index], storedIndexes[index]);
                    if (GUILayout.Button("x", GUILayout.Width(25)))
                    {
                        listNames.RemoveAt(index);
                        storedIndexes.RemoveAt(index);
        ...
        EndHorizontal();
        return (listNames, storedIndexes, listShortNames);
    }
