...
{
    public static class ReactionsSelector
    {
        internal static List<ReactionBase> SelectReactions
        ...
        private static List<ReactionBase> GetReactionsForAgent<TAgent, TReactionReason, TTableView, TTableContent>(TAgent thisAgent, TReactionReason reason, CharacterToPhenomContainerBase<TTableView, TTableContent> table, IAgent ag)
            where TAgent : SchoolAgentBase<TAgent>
            where TReactionReason : IReactionSource
            where TTableView : ViewDimensionBase<TTableContent>, new()
            where TTableContent : IReactionsCreator
        ...
        private static List<ReactionBase> GetInitializedReactions<TAgent, TReactionReason, TTableContent>(TAgent thisAgent, TReactionReason reason, TTableContent selector, CharacterTraitBase<TAgent, ReactionBase, FeatureBase, SchoolAgentStateBase<TAgent>, Sensor> trait)
            where TAgent : SchoolAgentBase<TAgent>
            where TReactionReason : IReactionSource
            where TTableContent : IReactionsCreator
        {
        ...
        private static string GetRelationName<TAgent>(RelationshipBase<TAgent, IAgent, SchoolAgentStateBase<TAgent>> rel)
            where TAgent:SchoolAgentBase<TAgent>
        {
            string relationKey;
            if (rel == null)
